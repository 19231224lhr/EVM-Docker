...
func TestCreateProof(t *testing.T) {
	...
	y_buf, proof_buf := wesolowski_go.GenerateVDF(seed, 10, 2048)
...
func TestVerifyProof(t *testing.T) {
	...
	assert.Equal(t, true, wesolowski_go.VerifyVDF(seed, append(y_buf, proof_buf...), 10, 2048), "must be true")
}
...
func TestVerifyProofSharedMemory(t *testing.T) {
	...
	assert.Equal(t, true, wesolowski_go.VerifyVDF(output[:len(seed)], output[len(seed):], 10, 2048), "must be true")
}
...
func TestGenerateAndVerifyIntSize(t *testing.T) {
	...
	for i := 1; i < 20; i++ {
		...
		iteration := i * 3
		y_buf, proof_buf := wesolowski_go.GenerateVDF(seed, iteration, int_size_bits)
		assert.Equal(t, true, wesolowski_go.VerifyVDF(seed, append(y_buf, proof_buf...), iteration, int_size_bits),
...
func TestGenerateAndVerifyProof(t *testing.T) {
	...
	for T := 5; T < 100; T++ {
		y_buf, proof_buf := wesolowski_go.GenerateVDF(seed, T, 2048)
		assert.Equal(t, true, wesolowski_go.VerifyVDF(seed, append(y_buf, proof_buf...), T, 2048), "failed when T = %d", T)
	}
}
...
func TestInterruptibleGenerator(t *testing.T) {
	...
	y_buf, proof_buf := wesolowski_go.GenerateVDFWithStopChan(seed, 10000, 2048, stop)
