...
func Execute(challenge int64, iterations int) []byte {
	...
	cmd1 := getCurrentAbPathByCaller() + "/vdf-cli " + strconv.FormatInt(challenge, 16) + " " + strconv.Itoa(iterations)
...
func Verify(challenge int64, iterations int, proof string) bool {
	...
	cmd1 := getCurrentAbPathByCaller() + "/vdf-cli " + strconv.FormatInt(challenge, 16) + " " + strconv.Itoa(iterations) + " " + proof
	cmd2 := "taskset -pc 0 $(pidof vdf-cli)"
