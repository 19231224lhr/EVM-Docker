...
// GetOutputChannel returns the wes_vdf output channel.
...
// IsFinished returns whether the wes_vdf execution is finished or not.
func (vdf *VDF) IsFinished() bool {
	return vdf.finished
}

// GetOutput returns the wes_vdf output, which can be bytes of 0s is the wes_vdf is not finished.
