...
func (l *PVSSLogic) PVSS(req *types.SecretPvssReq) (resp *types.SecretPvssRes, err error) {
	...
	case "3":
		...
		fmt.Println("DDDDDDDDDDDDDDDDDDDDDDDDDDD: ", pubPoly_string)
		fmt.Println("EEEEEEEEEEEEEEEEEEEEEEEE : ", "\"7/gv66krLc/2A1u81/EobWqyGAKFQXlxt05PSkSSZco=\"")
		...
		fmt.Println("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA : ", unmarshalled.B)
		if err != nil {
		...
		//fmt.Println("testtesttesttesttesttesttesttesttesttesttesttest")
		//// （3） 解密获得未加密的子秘密（分享者）（默认会进行一致性检验）
		//decShare1, _ := pvss.DecShare(suite, H, deserializedX[1], x[1], deserializedShares[1], restoredPubPoly)
		//fmt.Println(decShare1)
		//decShare2, _ := pvss.DecShare(suite, H, deserializedX[2], x[2], deserializedShares[2], restoredPubPoly)
		//fmt.Println(decShare2)
		//decShare3, _ := pvss.DecShare(suite, H, deserializedX[3], x[3], deserializedShares[3], restoredPubPoly)
		//fmt.Println(decShare3)
