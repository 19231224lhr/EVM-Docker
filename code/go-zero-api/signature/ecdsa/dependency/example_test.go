...
func Example_signMessage() {
	...
	privKey, pubKey := dependency.PrivKeyFromBytes(dependency.S256(), pkBytes)

	// Sign a message using the private key.
	message := "test message"
	messageHash := dependency.DoubleHashB([]byte(message))
...
func Example_verifySignature() {
	...
	pubKey, err := dependency.ParsePubKey(pubKeyBytes, dependency.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	signature, err := dependency.ParseSignature(sigBytes, dependency.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := dependency.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := dependency.PrivKeyFromBytes(dependency.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := dependency.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
